...
function tvDetails() {
  ...
  const { id } = useParams();
  const { info } = useSelector((state) => state.tv); // From store.
  ...
  return info ? (
    <>
      ...
      }} className='relative w-screen h-[180vh] md:h-[130vh] px-[8%]'>
      ...
      <a target='_blank' className='hover:text-[#6556cd]' href={info.detail.homepage}><i className="ri-external-link-fill"></i></a>
      ...
      <a target='_blank' className='hover:text-[#6556cd]' href={`https://www.imdb.com/title/${info.externalid.imdb_id}/`}>imdb</a>
      <Topnav />
      </nav>

      {/* PART 2 POSTER & DETAILS */}
      <div className='w-full flex flex-col md:flex-row'>
        <img className='h-[50vh] mb-4 md:pb-0 object-cover shadow-[8px_17px_38px_2px_rgba(0,0,0,0.5)]' src={`https://image.tmdb.org/t/p/original/${info.detail.poster_path || info.detail.backdrop_path}`} alt="" />
        ...
        <div className='content ml-[5%]'>
          ...
          <div className='flex flex-col md:flex-row items-start md:items-center pl-10 md:pl-0 gap-x-3 mt-2'>
            ...
            <h1>{info.detail.first_air_date.split("-")[0]} - {info.detail.last_air_date.split("-")[0]}</h1>
            <h1 className='text-yellow-300'>{info.detail.in_production ? "In Production" : ""}</h1>
      ...
      {/* PART 3 PROVIDERS */}
      <div className='w-[30vh] mt-4 md:mt-0'>
        ...
        {!info.watchproviders && <h1 className='mt-5 text-lg ml-3 font-semibold hover:text-yellow-300'> This Show is Currently Unavailable in India!</h1>}
      </div>
